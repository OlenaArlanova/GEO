"""
Download today's ChatGPT snapshot from Brightdata and log results to Google Sheets.

Usage:
    python download_snapshot.py                  # auto-pick today's latest snapshot
    python download_snapshot.py <snapshot_id>    # use a specific snapshot
"""
...
from query_llms import detect_brands, _extract_chatgpt_urls
...
def get_todays_chatgpt_snapshot():
    """Return the most recent ChatGPT snapshot created today, or None."""
    resp = requests.get(
    ...
    for snap in snapshots:
        if snap.get("dataset_id") != _CHATGPT_DATASET_ID:
            continue
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(f"Looking for today's ChatGPT snapshot ({TODAY})…")
        snap = get_todays_chatgpt_snapshot()
        if not snap:
            print("No ChatGPT snapshot found for today. Run the curl first.")
            sys.exit(1)
