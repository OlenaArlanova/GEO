"""
Download today's Perplexity snapshot from Brightdata and log results to Google Sheets.

Usage:
    python download_snapshot_perplexity.py                  # auto-pick today's latest snapshot
    python download_snapshot_perplexity.py <snapshot_id>    # use a specific snapshot
"""
...
_PERPLEXITY_DATASET_ID = "gd_m7dhdot1vw9a7gc1n"
_LLM_NAME = "Perplexity"
...
def get_todays_perplexity_snapshot():
    """Return the most recent Perplexity snapshot created today, or None."""
    resp = requests.get(
    ...
    for snap in snapshots:
        if snap.get("dataset_id") != _PERPLEXITY_DATASET_ID:
            continue
...
def _extract_text(item):
    md = item.get("exported_markdown")
    if md:
        return md
    html = item.get("answer_section_html")
    if html:
        return _strip_html(html)
    return ""


def _extract_perplexity_urls(item):
    urls = []
...
def _extract_prompt(item):
    ...
    return item.get("prompt") or (inp if isinstance(inp, str) else "") or ""
...
def process_snapshot(snapshot_id):
    ...
    for i, item in enumerate(data):
        ...
        prompt_text = _extract_prompt(item)
        ...
        urls = _extract_perplexity_urls(item)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(f"Looking for today's Perplexity snapshot ({TODAY})…")
        snap = get_todays_perplexity_snapshot()
        if not snap:
            print("No Perplexity snapshot found for today. Run the curl first.")
            sys.exit(1)
